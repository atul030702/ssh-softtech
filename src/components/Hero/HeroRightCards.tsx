...
const HeroCards: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="w-full max-w-6xl mx-auto mt-12 md:mt-16 px-4"
    >
        {/* 
        ...
        */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center relative group/cards perspective-1000">
            
            {/* Column 1 (Left) - Pushed down slightly */}
            <div className="flex flex-col gap-6 md:translate-y-8">
                <FeatureCard
                    variants={cardVariants}
                    icon={<Code className="text-blue-500 dark:text-blue-400" size={28} />}
            ...
            <motion.div
                ...
                className="w-full max-w-sm md:max-w-full aspect-square md:aspect-auto md:h-[420px] bg-linear-to-br from-brand-600 to-blue-700 dark:from-brand-600 dark:to-blue-800 rounded-3xl p-8 flex flex-col justify-center items-center text-center shadow-[0_20px_50px_-12px_rgba(14,165,233,0.4)] dark:shadow-[0_20px_50px_-12px_rgba(14,165,233,0.2)] border border-white/20 relative overflow-hidden group transition-all duration-500 md:scale-110 hover:shadow-[0_30px_60px_-10px_rgba(14,165,233,0.6)]"
            >
                {/* Background decoration */}
                <div className="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-full blur-3xl -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-700" />
                <div className="absolute bottom-0 left-0 w-40 h-40 bg-blue-500/30 rounded-full blur-3xl -ml-16 -mb-16 group-hover:scale-150 transition-transform duration-700 delay-100" />
                
                <motion.div 
                whileHover={{ rotate: 360, scale: 1.1 }}
                transition={{ duration: 0.8, type: "spring" }}
                className="relative z-10 bg-white/10 p-5 rounded-2xl mb-8 ring-1 ring-white/20 backdrop-blur-md shadow-lg"
                >
                <CheckCircle className="text-white w-12 h-12" />
                </motion.div>
                
                <h3 className="text-5xl md:text-6xl font-bold text-white mb-4 tracking-tight">200+</h3>
                <p className="text-blue-100 font-medium text-lg mb-8 max-w-[200px] leading-snug">Projects Delivered Successfully</p>
                
                <div className="flex items-center gap-2 text-sm font-medium text-white/90 bg-white/10 hover:bg-white/20 transition-colors px-5 py-2.5 rounded-full backdrop-blur-md border border-white/10 cursor-default">
                <Globe size={16} />
